...
if ([string]::IsNullOrWhiteSpace($AcrName) -and -not [string]::IsNullOrWhiteSpace($AzdEnvironment)) {
    Write-Header "Resolve ACR from azd environment"

    try {
        $AcrName = (Invoke-Azd -Args @("env", "get-value", "containerRegistryName", "-e", $AzdEnvironment, "--no-prompt") | Select-Object -First 1).Trim()
    }
    catch {
        Write-ErrorMsg "Could not resolve ACR name from azd environment '$AzdEnvironment': $_"
        exit 1
    }

    if ([string]::IsNullOrWhiteSpace($AcrName)) {
        Write-ErrorMsg "containerRegistryName is empty in azd environment '$AzdEnvironment'"
        exit 1
    }

    Write-Success "ACR from azd env: $AcrName"
}

if ([string]::IsNullOrWhiteSpace($AcrName)) {
    Write-ErrorMsg "AcrName is required. Pass -AcrName or use -AzdEnvironment with azd outputs available."
    exit 1
}
